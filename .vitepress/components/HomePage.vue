...
const copy = {
  en: {
    ...
    categoryLine: 'Qore turns provider events into readonly signals. Use the runtime with Qore DOM today, or bridge it into React through @qorejs/react.',
...
return h('p', {}, text(() => answer()))`,
    ...
    adapterLead: 'Install @qorejs/react and use React external store contract instead of copying chunks into component state.',
    adapterCode: `const answer = useQoreStream(
  () => stream(fetch('/api/chat').then((r) => r.body)),
  [prompt],
  { initialValue: '' }
)`,
    adapterStatus: 'Published adapter',
  ...
  zh: {
    ...
    categoryLine: 'Qore 把 provider event 转成 readonly signal。今天可以用 Qore DOM，已有 React 应用也可以通过 @qorejs/react 接入。',
    installLabel: '核心包',
    installCommand: 'pnpm add @qorejs/qore',
...
return h('p', {}, text(() => answer()))`,
    ...
    adapterLead: '安装 @qorejs/react，使用 React external store contract，不再把 chunk 手动复制进组件状态。',
    adapterCode: `const answer = useQoreStream(
  () => stream(fetch('/api/chat').then((r) => r.body)),
  [prompt],
  { initialValue: '' }

...
async function main() {
  ...
  try {
    ...
    await waitForText(page, 'Reactive streams. Native UI.')
    await waitForText(page, 'Reactive Stream Runtime')
    await waitForText(page, 'const events = stream.events(agent.run(task))')
    await waitForText(page, 'React / Vercel AI SDK: Token -> Hook state -> Component render -> Reconcile')
    await waitForText(page, 'Qore: Event -> Stream signal -> UI region')
    await waitForText(page, 'Stream one agent run without rewriting the transcript.')
    ...
    await waitForText(page, '响应式流。原生 UI。')
    await waitForText(page, 'Reactive Stream Runtime')
    await waitForText(page, 'const events = stream.events(agent.run(task))')
    await waitForText(page, 'React / Vercel AI SDK: Token -> Hook state -> Component render -> Reconcile')
    await waitForText(page, 'Qore: Event -> Stream signal -> UI region')

...
async function main() {
  ...
  try {
    ...
    await waitForText(page, 'Qore makes streams first-class state')
    await waitForText(page, 'Not an AI SDK. Not another UI framework.')
    ...
    await waitForText(page, 'Every token updates the bound node.')
    ...
    await waitForText(page, 'Reactive Stream Runtime')
    await waitForText(page, 'for AI-native interfaces')
    await waitForText(page, 'Qore 让 stream 成为一等状态')
    await waitForText(page, '不是 AI SDK，也不是又一个 UI framework')
    await waitForText(page, "const answer = stream(openai.chat('hello'))")
    ...
    await waitForText(page, '每个 token 只更新绑定节点。')
    await waitForText(page, 'Architecture')

...
async function main() {
  ...
  try {
    ...
    await waitForText(page, 'Turn streamed data into reactive UI.')
    await waitForText(page, 'Stable')
    await waitForText(page, 'v1.0.0')
    await waitForText(page, 'Choose a provider and stream one response.')
    await waitForText(page, 'OPENAI_API_KEY')
    await waitForText(page, 'Server-side OpenAI stream, one browser-side signal.')
    ...
    await waitForText(page, '把流式数据直接变成响应式 UI。')
    await waitForText(page, '稳定版')
    await waitForText(page, 'v1.0.0')
    await waitForText(page, 'OPENAI_API_KEY')
    await waitForText(page, 'OpenAI 留在服务端，浏览器只依赖一条 signal。')
    await page.getByRole('tab', { name: 'Anthropic', exact: true }).click()
    await waitForText(page, 'ANTHROPIC_API_KEY')
    await page.getByRole('tab', { name: 'Generic SSE', exact: true }).click()
    await waitForText(page, 'CUSTOM_AI_API_KEY')

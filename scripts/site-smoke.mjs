...
async function main() {
  ...
  try {
    ...
    await waitForText(page, 'No transcript rewrites, no state glue, no framework lock-in')
    ...
    await waitForText(page, 'One stream. Three surfaces. One runtime contract.')
    ...
    await waitForText(page, 'Reactive Stream Runtime')
    await waitForText(page, 'for AI-native interfaces')
    await waitForText(page, '让 stream 成为 AI-native interface 的一等状态')
    await waitForText(page, '不重写 transcript，不搬运状态，不锁定 framework')
    ...
    await waitForText(page, '一条 stream，三种 surface，一个稳定 runtime contract')
